...
namespace Microsoft.Graph
{
    ...
    public class BatchRequestContentCustom : HttpContent
    {
        /// <summary>
        ...
        public BatchRequestContentCustom(IBaseClient baseClient)
        ...
        [Obsolete("Please use the BatchRequestContentCollection for making batch requests as it supports handling more than 20 requests and provides a similar API experience.")]
        public BatchRequestContentCustom(IBaseClient baseClient, params BatchRequestStep[] batchRequestSteps): this(baseClient?.RequestAdapter ?? throw new ArgumentNullException(nameof(baseClient)), batchRequestSteps)
        {
        }
        ...
        /// <param name="batchRequestSteps">A list of <see cref="BatchRequestStep"/> to add to the batch request content.</param>
        [Obsolete("Please use the BatchRequestContentCollection for making batch requests as it supports handling more than 20 requests and provides a similar API experience.")]
        public BatchRequestContentCustom(IRequestAdapter requestAdapter, params BatchRequestStep[] batchRequestSteps)
        {
            ...
            if (batchRequestSteps.Count() > CoreConstants.BatchRequest.MaxNumberOfRequests)
                throw new ArgumentException(string.Format("{0} exceeds the maximum value of {1}.", "Number of batch request steps", CoreConstants.BatchRequest.MaxNumberOfRequests));
            ...
            foreach (BatchRequestStep requestStep in batchRequestSteps)
            {
                ...
                {
                    throw new ArgumentException("Corresponding batch request id not found for the specified dependsOn relation.");
                }
...
#pragma warning restore CS0618
        ...
        internal async Task<Stream> GetBatchRequestContentAsync(CancellationToken cancellationToken = default)
        {
            ...
            using (var writer = new Utf8JsonWriter(stream))
            {
                ...
                writer.WritePropertyName("requests");// requests property name
        ...
        private async Task WriteBatchRequestStepAsync(BatchRequestStep batchRequestStep, Utf8JsonWriter writer, CancellationToken cancellationToken)
        {
            ...
            writer.WriteString("id", batchRequestStep.RequestId);//write the id property
            writer.WriteString("url", GetRelativeUrl(batchRequestStep.Request.RequestUri));//write the url property
            writer.WriteString("method", batchRequestStep.Request.Method.Method);// write the method property
